...
params ["_units","_pos",["_dir",0],["_height",3500],["_waitBeforeJump",40],["_giveParachute",false],["_onSceneCut",{}],["_onLanding",{}]];
...
{
    _planeObjects = [_pos getPos [_forEachIndex * 100,_dir + 90],_dir,_height] call grad_drop_fnc_spawnPlane;
    ...
    [_plane,_x,_soundPlane,_jm,_planeLights,_waitBeforeJump,_giveParachute,_onLanding] spawn {
        params ["_plane","_x","_soundPlane","_jm","_planeLights","_waitBeforeJump","_giveParachute","_onLanding"];

        sleep 4;

        [_plane,_x,_giveParachute] call grad_drop_fnc_moveInPlane;
        [_x,_plane,_soundPlane,_jm,_planeLights,_waitBeforeJump,_onLanding] spawn grad_drop_fnc_startProcedure;
    };
