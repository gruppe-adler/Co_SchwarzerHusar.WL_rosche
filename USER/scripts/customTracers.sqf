...
if (_activated) then {
    if (isserver) then {
        ...
        while {!isnull _gunner && !isnull _logic} do {
            if ({_gunner distance _x < 100} count (playableunits + switchableunits) == 0) then {
                _delay = 0.05 + random 0.1;

                private _target = call compile (_logic getVariable ["Target","objNull"]);
                if(isNil "_target")then{_target = objNull};
        ...
        waituntil {!isnull (_logic getvariable ["bis_fnc_moduleTracers_gunner",objnull])};
        _gunner = _logic getvariable ["bis_fnc_moduleTracers_gunner",objnull];
        _gunner hideobject true;
    };
} else {
    if (isserver) then {
